...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if ($email_or_phone && $password) {
        ...
        if ($is_email) {
            ...
            $stmt = $conn->prepare("SELECT id, name, email, password FROM login WHERE phonenumber = ?");
            $stmt->bind_param("s", $email_or_phone);
        }
        ...
        $login_error = 'Please enter both email/phone and password.';
    }
}
...
<body>
    <div class="login-wrapper">
        ...
        <form method="POST" action="index2.php" autocomplete="off">
            ...
            <input type="text" id="email_or_phone" name="email_or_phone" required autofocus placeholder="Enter your email or phone number" />
        ...
        <p style="margin-top: 1rem; color: #ddd;">
            ...
            <a href="login.php" style="color: #4a90e2; text-decoration: none; font-weight: 600;">Register here</a>
        </p>
        <p style="margin-top: 0.5rem; color: #ddd; font-size: 0.9rem; opacity: 0.8;">
        ...
        <div class="container">
            <div class="header-content">
                ...
                <nav>
                    ...
                    <?php if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true): ?>
                        <li><span style="color: var(--primary-color); font-weight: 600;">Welcome, <?php echo htmlspecialchars($_SESSION['username']); ?></span></li>
    ...
    <div id="applicationModal" style="display:none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.6);">
        <div style="background-color: #fff; margin: 5% auto; padding: 20px; border-radius: 15px; width: 90%; max-width: 600px; box-shadow: 0 10px 30px rgba(0,0,0,0.3); position: relative; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            ...
            <form id="applicationForm" method="POST" action="index2.php" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1rem;">
                ...
                <input type="text" id="applicantName" name="applicant_name" required placeholder="Enter your full name" 
                ...
                <input type="email" id="applicantEmail" name="applicant_email" required placeholder="Enter your email address" 
                       value="<?php echo isset($_SESSION['email']) ? htmlspecialchars($_SESSION['email']) : ''; ?>" 
                       style="padding: 0.75rem; border-radius: 8px; border: 1px solid #ccc; font-size: 1rem;">
                
                <label for="applicantPhone">Phone Number</label>
                <input type="tel" id="applicantPhone" name="applicant_phone" required placeholder="Enter your phone number" 
                       value="<?php 
                           if (isset($_SESSION['user_id'])) {
                               $phone_stmt = $conn->prepare("SELECT phonenumber FROM login WHERE id = ?");
                               $phone_stmt->bind_param("i", $_SESSION['user_id']);
                               $phone_stmt->execute();
                               $phone_result = $phone_stmt->get_result();
                               if ($phone_result->num_rows === 1) {
                                   $phone_data = $phone_result->fetch_assoc();
                                   echo htmlspecialchars($phone_data['phonenumber']);
                               }
                               $phone_stmt->close();
                           }
                       ?>" 
