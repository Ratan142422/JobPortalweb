...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if ($email_or_phone === $valid_recruiter_username && $password === $valid_recruiter_password) {
        $_SESSION['recruiter_loggedin'] = true;
        $_SESSION['recruiter_username'] = $email_or_phone;
        header("Location: index1.php");
        exit();
    } else {
        // Try to login with registered user data
        $db_host = 'localhost';
        $db_user = 'root';
        $db_pass = '';
        $db_name = 'job_portal';
        ...
        if (!$conn->connect_error) {
            ...
            if ($is_email) {
                ...
                $stmt = $conn->prepare("SELECT id, name, email, password FROM register WHERE phonenumber = ?");
                $stmt->bind_param("s", $email_or_phone);
            }
            ...
            if ($result->num_rows === 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    $_SESSION['recruiter_loggedin'] = true;
                    $_SESSION['recruiter_username'] = $user['name'];
